...
- Limitation des erreurs HTTP journalisées au statut de réponse pour protéger le contenu
  retourné par les services externes.
...
- Vérification de l’accès aux données Tempo après l’authentification lors de la configuration,
  reconfiguration et réauthentification, avec un message dédié en cas d’accès refusé.
